// 生成gb2260.json的脚本
//
// 运行：npx ts-node src/shared/m3/types/gb2260.gen.ts
// stdSrc来自 https://www.mca.gov.cn/article/sj/xzqh/2020/20201201.html
// hkMacaoSrc来自(数据未获得官方认可仅供参考不处理)
//    https://github.com/cn/GB2260/blob/develop/contrib/hongkong-201512.tsv
...
810000	香港特别行政区						
820000	澳门特别行政区`
const hkMacaoSrc = `810000	香港特别行政区
810100	香港岛
810101	中西区
810102	湾仔区
810103	东区
...
for(const patchItem of patch) {
  const found = kv.findIndex(kv => kv[0] === patchItem[0])
  if(found >= 0) {
    kv[found] = patchItem
  } else {
    kv.push(patchItem)
...
// 创建节点的函数
function toKv(item: string[], children?:CodeNode[]){
  // 把(已撤销)的放在后面
  if(children) {
    const removed:any = [];
    const remain:any = []
    for(const ch of children) {
      if(ch.label.indexOf("(已撤销)") >= 0) {
        removed.push(ch);
      } else {
        remain.push(ch)
      }
    }
    children = [...remain, ...removed];
  }
  ...
  return {
    ...
    children,
    disabled: item[1].indexOf('(已撤销)')>0 ? true : undefined
  } as CodeNode
}
